...
\begin{section}{\glsfmtfull{ris}}
	...
	\begin{subsection}{Wave Scattering Models}
		\begin{subsubsection}{Principles}\label{sc:principles}
			...
			Specifically,
			\begin{itemize}
				\item \emph{Structural component:} Depends on the geometry and material of the scatterer. It is usually modelled as part of the environment multipath \cite{Thomas2012,Liang2020} or a baseband \gls{dc} offset when the impinging signal is \gls{cw} \cite{Boyer2014}.
		...
		\begin{subsubsection}{\glsfmtfull{bd} Model}
			...
			From a network theory perspective \cite{Ivrlac2010}, a general \gls{bd}-\gls{ris} can be modeled as an $N_\mathrm{S}$-port network that divides into $G$ individual groups, each containing $L \triangleq N_\mathrm{S} / G$ elements interconnected by real-time reconfigurable connections.
			...
			Fig. \ref{fg:bd_ris_architecture_2} illustrates an 8-element \gls{ris} with group size 2 (left) and 4 (right).
			\begin{figure}[H]
				\centering
			...
			% thus redistribute both amplitude and phase of the scattered wave with zero power loss.
			% \begin{figure}[H]
			% 	\centering
			% 	\resizebox{0.75\columnwidth}{!}{
			...
			\begin{subequations}
				...
				\end{equation}
			\end{subequations}
			On the other hand, lossless networks may also be built over asymmetric passive components (e.g., ring hybrids and branch-line hybrids) \cite{Ahn2006} such that the symmetric constraint \eqref{eq:symmetric_property} is unnecessary.
			This corresponds to the ultimate passive model where energy conservation \eqref{eq:unitary_property} is the only constraint for each group, as has been widely adopted in quantum physics.
	...
	\begin{subsection}{Non-Linear Harvester Behavior}
		...
		\begin{subsubsection}{Operation Regions and Signal Models}\label{sc:operation_regions}
			...
			\begin{itemize}
				...
				\begin{equation}
					i_\mathrm{out} = \sum_{i=0}^\infty k_i R_\mathrm{A}^{i/2} \mathbb{A}\bigl\{ y^i(t) \bigr\} = \sum_{i=0,\text{even}}^\infty k_i R_\mathrm{A}^{i/2} \mathbb{A}\bigl\{ y^i(t) \bigr\}
					\label{eq:dc_current_expansion}
				\end{equation}
				where the second equality is because $\mathbb{A}\bigl\{y^i(t)\bigr\} = 0$ for odd $i$. Note that the dependency of $k_i$ on $-v_\mathrm{out}=-i_\mathrm{out} R_\mathrm{L}$ makes it nontrivial to formulate a closed-form expression for the harvested \gls{dc} power.
...
\begin{section}{\glsfmtfull{swipt}}\label{sc:swipt}
	...
	\begin{subsection}{\glsfmtfull{r-e} Trade-off}
...
\begin{section}{\glsfmtfull{bc}}
	...
	\begin{subsection}{Applications}
		\begin{figure}[H]
			...
			\subfloat[Monostatic backscatter]{
				\resizebox{0.48\linewidth}{!}{
					\input{assets/chapter_2/mbc.tex}
				}
				\label{fg:mbc_intro}
			}
			\subfloat[Bistatic backscatter]{
				\resizebox{0.48\linewidth}{!}{
					\input{assets/chapter_2/bbc.tex}
				}
				\label{fg:bbc_intro}
			}
			\\
			\subfloat[Ambient backscatter]{
				\resizebox{0.48\linewidth}{!}{
					\input{assets/chapter_2/ambc.tex}
				}
				\label{fg:ambc_intro}
			}
			\subfloat[Symbiotic radio]{
				\resizebox{0.48\linewidth}{!}{
					\input{assets/chapter_2/sr.tex}
				}
				\label{fg:sr_intro}
			}
			\caption{
		...
		\begin{subsubsection}{\glsfmtfull{mbc}}
			...
			As shown in Figs. \subref*{fg:backcom_monostatic} and \subref*{fg:mbc_intro}, the interrogator transmits a \gls{cw} in the downlink and receives the reflected information-bearing signal in the uplink.
		...
		\begin{subsubsection}{\glsfmtfull{bbc}}
			\gls{bbc} separates the carrier emitter and reader into two individual devices, as illustrated in Figs. \subref*{fg:backcom_bistatic} and \subref*{fg:bbc_intro}.
		...
		\begin{subsubsection}{\glsfmtfull{ambc}}
			...
			As shown in Fig. \subref*{fg:ambc_intro}, it gets rid of dedicated \gls{rf} sources and relies on ambient signals (e.g., TV, FM, and Wi-Fi) for both energy harvesting and information transfer.
		...
		\begin{subsubsection}{\glsfmtfull{sr}}
			...
			As shown in Fig. \subref*{fg:sr_intro}, it shares not only the transmitter, \gls{rf} source, but also the receiver with a primary transmission.
