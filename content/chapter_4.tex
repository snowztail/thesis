...
\begin{section}{Introduction}
	...
	The receiver can be either co-located or separated with the carrier emitter, known as \gls{mbc} and \gls{bbc} in Fig.~\subref*{fg:mbc} and \subref*{fg:bbc}, respectively.
	...
	\end{enumerate}
	The second property motivated \cite{Long2020a,Liang2020,Guo2019b,Ding2020,Hassani2023,Zhou2019a,Wu2021a,Xu2021a,Yang2021a,Yang2018,Han2021,Zhang2022,Dai2023} to view \gls{sr} as a multiplicative \gls{noma}\footnote{As discussed in Section \ref{sc:mcs}, a time-varying reflection pattern at rate $\Delta f$ can shift the frequency of the scattered signal by $\pm \Delta f$. Therefore, the spreading ratio should be carefully chosen to balance backscatter rate and frequency offset.} and perform \gls{sic} from primary to backscatter link.
	During primary decoding, the backscatter signal can be modeled as channel uncertainty or multiplicative interference, when the spreading factor is large or small, respectively.
	...
	Thanks to double modulation, backscatter detection can be viewed as part of channel training, and the impact of backscatter modulation can be modeled as dynamic passive beamforming afterwards.
...
\begin{section}{RIScatter}
	...
	\begin{subsection}{Principles}
		...
		As shown in Fig.~\subref*{fg:input_distribution}, the reflection pattern of each RIScatter node over time is semi-random and guided by the input probability assigned to each state.
		...
		Fig.~\subref*{fg:time_structure} illustrates the reflection pattern of a RIScatter node over time, where ``PB'', ``BB'', and ``CB'' refer to primary symbol block, backscatter symbol block, and channel block, respectively.
		...
		\begin{remark}
			...
			Upon successful backscatter detection, the impact of RIScatter nodes on the primary link can be modeled as dynamic passive beamforming.
			\label{rm:dynamic_passive_beamforming}
		\end{remark}
	...
	\begin{subsection}{System Model}
		...
		We assume the channel coherence time is longer than the backscatter symbol duration, the primary symbol duration is much longer than multipath delay spread (i.e., no inter-symbol interference), and the signal going through two or more RIScatter nodes is too weak to be received by the user.
...
\begin{section}{Rate-Region Characterization}
	...
	\begin{maxi!}
		{\scriptstyle{\{\mathbf{p}_k\}_{k \in \mathcal{K}},\mathbf{w},\mathbf{t}}}{I(x_{\mathcal{K}})}{\label{op:weighted_sum_rate}}{\label{ob:weighted_sum_rate}}
		\addConstraint{\mathbf{1}^\mathsf{T} \mathbf{p}_k}{=1,}{\quad \forall k}{\label{co:sum_probability}}
		\addConstraint{\mathbf{p}_k}{\ge \mathbf{0},}{\quad \forall k}{\label{co:nonnegative_probability}}
		\addConstraint{\lVert \mathbf{w} \rVert^2}{\le P}{\label{co:transmit_power}}
		\addConstraint{t_{l-1}}{\le t_l,}{\quad \forall l}{\label{co:sequential_threshold}}
		\addConstraint{\mathbf{t}}{\ge \mathbf{0},}{\label{co:nonnegative_threshold}}
	\end{maxi!}
	where $\mathbf{p}_k = [p(x_{1_k}), \ldots, p(x_{M_k})]^\mathsf{T} \in \mathbb{I}^M$ is the input distribution of node $k$.
	...
	The original problem is highly non-convex and we propose a \gls{bcd} algorithm that iteratively updates $\{\mathbf{p}_k\}_{k \in \mathcal{K}}$, $\mathbf{w}$ and $\mathbf{t}$.
	...
	\begin{subsection}{Input Distribution}
		...
		\begin{maxi!}
			{\scriptstyle{\{\mathbf{p}_k\}_{k \in \mathcal{K}}}}{I(x_{\mathcal{K}})}{\label{op:input_distribution}}{}
		...
		\begin{algorithm}[!t]
			...
			\begin{algorithmic}[1]
				\Require $K$, $N$, $\mathbf{h}_{\text{D}}^\mathsf{H}$, $\mathbf{H}_{\text{C}}$, $\mathbf{\alpha}$, $\mathcal{X}$, $\sigma_v^2$, $\rho$, $\mathbf{w}$, $\mathbf{t}$, $\epsilon$
				\Ensure $\{\mathbf{p}_k^\star\}_{k \in \mathcal{K}}$
				...
				\State \phantom{Initialize} $\mathbf{p}_k^{(0)} > \mathbf{0}$, $\forall k$
				...
				\Repeat
					...
					\State \phantom{Update} $\mathbf{p}_k^{(r)}$, $\forall k$ by \eqref{eq:input_kkt_solution}
		...
		\begin{remark}
			...
			\label{rm:input_kkt_distribution}
		\end{remark}
	\end{subsection}
	...
	\begin{subsection}{Active Beamforming}
		...
		\begin{maxi!}
			{\scriptstyle{\mathbf{w}}}{I(x_{\mathcal{K}})}{\label{op:active_beamforming}}{\label{ob:active_beamforming}}
	...
	\begin{subsection}{Decision Threshold}
		...
		\begin{maxi!}
			{\scriptstyle{\mathbf{t}}}{I(x_{\mathcal{K}})}{\label{op:decision_threshold}}{\label{ob:decision_threshold}}
		...
		\begin{maxi!}
			{\scriptstyle{\mathbf{t} \in \mathcal{T}^{L+1}}}{I_{\text{B}}(x_{\mathcal{K}};\hat{x}_{\mathcal{K}})}{\label{op:decision_threshold_discrete}}{\label{ob:decision_threshold_discrete}}
...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Comparison of Scattering Applications}
		...
		When $N$ is moderate, the randomly scattered signals should be modeled as interference rather than stable multipath, and the \gls{sr} point should move vertically towards the \gls{ambc} point.
	...
	\begin{subsection}{Input Distribution under Different \glsfmtshort{qos}}
		...
		\end{figure}
		The objective is to demonstrate that RIScatter nodes can leverage \gls{csi}- and \gls{qos}-adaptive input distribution design to balance backscatter modulation and passive beamforming.
	...
	\begin{subsection}{Rate Region under Different Configurations}
		...
		\begin{subsubsection}{Imperfect \glsfmtshort{csi}}
		...
		\begin{subsubsection}{Primary \glsfmtshort{snr}}
			...
			It suggests that the nodes should be positioned in the proximity of the user to avoid \gls{snr} imbalance.
		...
		\begin{subsubsection}{Backscatter \glsfmtshort{snr}}
