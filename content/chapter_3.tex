		...
		\begin{figure}[!t]
			...
			\subfloat[\gls{ts} receiver\label{fi:ts_receiver}]{
				\resizebox{0.9\columnwidth}{!}{
					\input{assets/chapter_3/ts_receiver.tex}
				}
			}
			\\
			\subfloat[\gls{ps} receiver\label{fi:ps_receiver}]{
				\resizebox{0.9\columnwidth}{!}{
					\input{assets/chapter_3/ps_receiver.tex}
				}
			}
...
\begin{section}{Problem Formulation}\label{se:problem_formulation}
	...
	\begin{maxi!}
		{\scriptstyle{\mathbf{\phi},\mathbf{w}_{\mathrm{I}},\mathbf{w}_{\mathrm{P}},\rho}}{z(\mathbf{\phi},\mathbf{w}_{\mathrm{I}},\mathbf{w}_{\mathrm{P}},\rho)}{\label{op:original}}{\label{ob:original}}
		\addConstraint{R(\mathbf{\phi},\mathbf{w}_{\mathrm{I}},\rho) \ge \bar{R}}\label{co:original_rate}
		...
		\addConstraint{\lvert{\mathbf{\phi}}\rvert=\mathbf{1}}\label{co:original_modulus}
	...
	\begin{subsection}{Passive Beamforming}
		...
		\end{figure*}
		\begin{maxi!}
			{\scriptstyle{\mathbf{\Phi}}}{\tilde{z}(\mathbf{\Phi})}{\label{op:irs}}{\label{ob:irs}}
			\addConstraint{R(\mathbf{\Phi}) \ge \bar{R}}\label{co:irs_rate}
			\addConstraint{\mathrm{diag}^{-1}(\mathbf{\Phi})=\mathbf{1}}\label{co:irs_modulus}
			\addConstraint{\mathbf{\Phi}\succeq{\mathbf{0}}}\label{co:irs_sd}
			\addConstraint{\mathrm{rank}(\mathbf{\Phi})=1.\label{co:irs_rank}}
		\end{maxi!}
	...
	\begin{subsection}{Block Coordinate Descent}
		...
		\begin{algorithm}[!t]
			...
			\begin{algorithmic}[1]
				...
				\State \textbf{Initialize} $i \gets 0$, $\mathbf{\phi}^{(0)}$, $\mathbf{b}_{\mathrm{I/P},n}^{(0)}$, $\mathbf{s}_{\mathrm{I/P}}^{(0)}$, $\forall n$
				\State Set $\mathbf{w}_{\mathrm{I/P},n}^{(0)}$, $\forall n$ by \eqref{eq:w}
				\State Compute $R^{(0)}$, $z^{(0)}$ by \eqref{eq:R_waveform}, \eqref{eq:z_waveform}
				\Repeat
					\State $i \gets i + 1$
					\State Get $\mathbf{\phi}^{(i)}$ based on $\mathbf{w}_{\mathrm{I/P}}^{(i-1)}$ by Algorithm~\ref{al:m_sca}
					\State Update $\mathbf{h}_n^{(i)}$, $\mathbf{b}_n^{(i)}$, $\forall n$ by \eqref{eq:h_n}, \eqref{eq:b_n}
					\State Update $\mathbf{s}_{\mathrm{I}}^{(i)}$, $\mathbf{s}_{\mathrm{P}}^{(i)}$ by \eqref{eq:s_i}, \eqref{eq:s_p}
					\State Update $\mathbf{w}_{\mathrm{I/P},n}^{(i)}$, $\forall n$ by \eqref{eq:w}
					\State Compute $R^{(i)}$, $z^{(i)}$ by \eqref{eq:R_waveform}, \eqref{eq:z_waveform}
					\If{$\rho=0$}
				...
				\State Set $\mathbf{\phi}^{\star} \gets \mathbf{\phi}^{(i)}$, $\mathbf{w}_{\mathrm{I/P}}^{\star} \gets \mathbf{w}_{\mathrm{I/P}}^{(i)}$
				\State \textbf{Output} $\mathbf{\phi}^{\star}$, $\mathbf{w}_{\mathrm{I}}^{\star}$, $\mathbf{w}_{\mathrm{P}}^{\star}$
			\end{algorithmic}
		\end{algorithm}
	\end{subsection}
\end{section}
