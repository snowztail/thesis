...
\begin{section}{Background and Motivation}
	...
	Before delving into these questions, we first provide a short overview of \gls{ris} and introduce some potential applications in the next section.
...
\begin{section}{Overview on \glsfmtfull{ris}}
	...
	\begin{subsection}{Concept}
		...
		The bottom later is a circuit board that associate each element with adjustable components like varactor and \gls{pin} diodes \cite{Dai2020}.
		It also hosts a \gls{fpga} controller that controls the circuit and coordinate with other devices in the network.
		By adjusting the scatter response of each element, the \gls{ris} can effectively manipulate the wavefront and thus improve the ambient wireless environment.
		\begin{figure}
			\centering
			\includegraphics[width=0.9\textwidth]{ris_architecture.pdf}
			\caption{A typical architecture of \gls{ris} \cite{Wu2020}.}
	...
	\begin{subsection}{Characteristics}
		...
		\begin{itemize}
			...
			\item \emph{Flexible:} It provides a software configurable environment that can be adapted for different applications and scenarios. This is different from reflectarray \cite{Nayeri2018} and frequency selective surfaces \cite{Anwar2018} with predefined radiation patterns and frequency response.
			\item \emph{Full-duplex and universal:} This physical-layer solution can simultaneously benefit downlink and uplink thanks to channel reciprocity \cite{Wu2021}, and simultaneously support communication, sensing, and power transfer without self-interference. This is different from \gls{df} relays that are designed for a specific communication link and suffers from packet delays.
	...
	% This disruptive technology can be introduced to legacy networks for signal enhancement, interference suppression, scattering enrichment, blockage bypassing, coverage extension, and security control.
	\end{subsection}
\end{section}
